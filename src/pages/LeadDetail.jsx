...
export default function LeadDetail() {
  ...
  const loadData = async () => {
    ...
    if (l?.prospectId) {
      ...
      // Auto-select message type based on messageCount
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-sm text-slate-500 flex-wrap">
              ...
              <span className="flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5" />{lead.messageCount || 0} message(s) envoyé(s)</span>
            </div>
          </div>
        ...
        {generatedMsg && (
          <div className="space-y-3">
            ...
            <div className="p-3 bg-slate-50 rounded-lg border">
              ...
              <div className="text-sm text-slate-800 whitespace-pre-wrap leading-relaxed">{generatedMsg.body}</div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <CopyButton text={(generatedMsg.subject ? `Sujet: ${generatedMsg.subject}\n\n` : "") + generatedMsg.body} label="Copier" />
              <Button
                onClick={handleMarkSent}
                disabled={isMarking}
                className="bg-green-600 hover:bg-green-700 gap-2"
              >
                {isMarking ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
                {isMarking ? "Enregistrement…" : "Marquer comme envoyé"}
              </Button>
              <p className="text-xs text-slate-400">Copiez d'abord, puis cliquez après envoi manuel</p>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="space-y-3">
            {messages.filter(m => m.status === "SENT").map(m => (
              <div key={m.id} className="p-3 bg-slate-50 rounded-lg border text-sm">
                ...
                {m.subject && <div className="text-xs font-medium text-slate-600 mb-1">Sujet: {m.subject}</div>}
                <div className="text-xs text-slate-600 line-clamp-3">{m.body}</div>
              </div>
            ))}
          </div>
        </div>
